...
public class ProductService(IProductRepository productRepository, ICategoryRepository categoryRepository, ICompanyRepository companyRepository) : IProductService
{
	...
	public async Task<PageBaseResponse<IList<ProductDto>>> GetPaginatedProduct(int page, int pageSize, string filter = null)
	{
		return await _productRepository.GetPaginatedProductAsync(x => x.Name.Contains(filter) || filter == null, page, pageSize);
